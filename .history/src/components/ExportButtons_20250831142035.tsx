...
const ExportButtons = ({ leads }: ExportButtonsProps) => {
  ...
  const exportToExcel = async () => {
    ...
    // Try dynamic import of xlsx (sheetjs). If not available, fall back to CSV and notify.
    try {
      // Use the browser-ready ESM build so Vite can resolve it reliably during dev and build.
      ...
      // If import failed (package not installed or bundler blocked dynamic import),
      // fall back to CSV but with .xlsx filename and warn the user.
      console.warn("xlsx export failed, falling back to CSV. Install 'xlsx' for real xlsx export.", err);
      const headers = [
        'ID','Name','Title','Company','Email','Phone','LinkedIn Profile',
        'Corporate LinkedIn','Corporate Website','Location','Industry','Company Size','Score','Additional Data'
      ];
      ...
      try {
        ...
        (window as any).alert?.("Real Excel export requires the 'xlsx' package. Falling back to CSV-format file with .xlsx extension.");
      } catch {}
    }
  };
